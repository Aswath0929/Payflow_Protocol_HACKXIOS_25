...
function ContractCard({
  ...
  return (
    <div className="relative group">
      <div
        className={`absolute -inset-0.5 bg-gradient-to-r ${gradients[variant]} rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-300`}
      />
      <div className="relative bg-gray-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-5 h-full">
        ...
        <div className="flex items-center gap-2 mb-4 p-3 bg-white/5 rounded-xl">
          <code className="text-xs font-mono text-gray-300 flex-1 truncate">{address}</code>
          <button onClick={onCopy} className="p-2 hover:bg-white/10 rounded-lg transition-colors" title="Copy address">
            {isCopied ? "✅" : "📋"}
          </button>
          <button
            onClick={onEtherscan}
        ...
        <div className="flex flex-wrap gap-2">
          {features.map((feature, idx) => (
            <span key={idx} className="px-2 py-1 bg-white/5 text-xs text-gray-400 rounded-lg">
              {feature}
            </span>
          ))}
        </div>
