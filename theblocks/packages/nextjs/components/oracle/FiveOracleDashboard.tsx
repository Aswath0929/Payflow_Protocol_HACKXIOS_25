  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading
          ? Array.from({ length: 5 }).map((_, i) => <OracleCardSkeleton key={i} />)
...
function OracleCard({ oracle, isSelected }: { oracle: OracleData; isSelected?: boolean }) {
  ...
  return (
    <div className="relative group">
      <div
        className={`absolute -inset-0.5 bg-gradient-to-r ${colors.glow} rounded-2xl blur ${isSelected ? "opacity-60" : "opacity-30"} group-hover:opacity-50 transition duration-300`}
      />
      <div
        className={`relative bg-gradient-to-br ${colors.bg} ${colors.border} border rounded-2xl p-5 backdrop-blur-xl ${isSelected ? "ring-2 ring-green-500/50" : ""}`}
      >
        <div className="flex items-center justify-between mb-4">
          ...
          {isSelected && (
            <span className="px-2 py-1 bg-green-500/20 text-green-300 rounded-full text-xs">✓ Selected</span>
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-gray-400">{oracle.status !== "error" ? `${ageStr} ago` : "--"}</span>
          </div>

          {oracle.confidence !== undefined && oracle.confidence > 0 && (
            <div className="text-xs text-gray-400">±${oracle.confidence.toFixed(2)} confidence interval</div>
          )}
        </div>
      </div>
    </div>
