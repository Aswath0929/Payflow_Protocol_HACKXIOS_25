  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative group">
        <div
          className={`absolute -inset-0.5 bg-gradient-to-r ${priceChange >= 0 ? "from-green-600 to-emerald-600" : "from-red-600 to-orange-600"} rounded-2xl blur opacity-30 group-hover:opacity-50 transition duration-300`}
        />
        <div className="relative bg-gray-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
          ...
          <div className="flex items-end gap-4 mb-6">
            <span className="text-4xl font-bold font-mono">{currentPrice ? formatPrice(currentPrice) : "--"}</span>
            <div
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-3 bg-white/5 rounded-xl">
              ...
              <p className="font-mono font-bold text-green-400">{highPrice ? formatPrice(highPrice) : "--"}</p>
            </div>
            <div className="text-center p-3 bg-white/5 rounded-xl">
              <p className="text-xs text-gray-400 mb-1">24h Low</p>
              <p className="font-mono font-bold text-red-400">{lowPrice ? formatPrice(lowPrice) : "--"}</p>
            </div>
            <div className="text-center p-3 bg-white/5 rounded-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <MiniPriceCard symbol="BTC" name="Bitcoin" price={68432.5} change={2.34} icon="₿" color="orange" />
        <MiniPriceCard symbol="LINK" name="Chainlink" price={14.78} change={-1.23} icon="⬡" color="blue" />
        <MiniPriceCard symbol="SOL" name="Solana" price={175.42} change={5.67} icon="◎" color="purple" />
      </div>
