...
export function OracleDashboard() {
  ...
  const api3Prices = prices.filter(p => p.source === "API3");
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatsCard
          ...
          value="4"
          subtitle="Chainlink • Pyth • API3 • Synced"
          icon="📡"
          color="cyan"
        />
        <StatsCard
          ...
          color="purple"
        />
        <StatsCard title="BFT Consensus" value="3/5" subtitle="Byzantine threshold" icon="🛡️" color="green" />
        <StatsCard
          title="Last Update"
          ...
          <div className="relative bg-gray-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {loading ? (
                ...
                chainlinkPrices.map((price, idx) => <PriceRow key={idx} data={price} />)
              )}
            </div>
          </div>
        </div>
          ...
          <div className="relative bg-gray-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {loading ? (
                ...
                pythPrices.map((price, idx) => <PriceRow key={idx} data={price} showConfidence />)
              ) : (
        ...
        <div className="relative bg-gray-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-lg">API3</h3>
                <p className="text-xs text-gray-400">dAPI Oracle • Sepolia Testnet</p>
              </div>
            </div>
            <div className="flex items-center gap-2 px-3 py-1 bg-red-500/20 rounded-full">
              <div className="w-2 h-2 bg-red-400 rounded-full animate-pulse" />
              <span className="text-xs text-red-300">Adversarial Condition</span>
            </div>
          </div>
          ...
          <div className="mb-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
            <div className="flex items-start gap-3">
              <span className="text-2xl">🔬</span>
              <div>
                <p className="font-medium text-yellow-300 text-sm">Hackathon Adversarial Condition</p>
                <p className="text-xs text-yellow-200/70 mt-1">
                  API3 on Sepolia returns stale data (&gt;24h old). Our BFT system automatically detects and excludes
                  stale oracles, demonstrating fault-tolerance in action.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-3">
              {loading ? (
                ...
                api3Prices.map((price, idx) => <PriceRow key={idx} data={price} />)
              ) : (
                <div className="text-center py-4 text-gray-500">
                  <p>No API3 data available</p>
                </div>
              )}
            </div>
            <div className="bg-white/5 rounded-xl p-4">
              <p className="text-sm text-gray-400 mb-2">Oracle Status</p>
              {api3Prices.length > 0 && api3Prices[0].status === "error" ? (
                <div className="flex items-center gap-2">
                  <span className="text-red-400 text-2xl">❌</span>
                  <div>
                    <p className="font-medium text-red-300">EXCLUDED</p>
                    <p className="text-xs text-gray-500">Data too stale for BFT consensus</p>
                  </div>
                </div>
              ) : api3Prices.length > 0 && api3Prices[0].status === "stale" ? (
                <div className="flex items-center gap-2">
                  <span className="text-yellow-400 text-2xl">⚠️</span>
                  <div>
                    <p className="font-medium text-yellow-300">DEGRADED</p>
                    <p className="text-xs text-gray-500">Data stale but within threshold</p>
                  </div>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Contributing to consensus</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {["Chainlink", "Pyth", "Synced"].map((source, idx) => {
              ...
              return (
                <div key={idx} className="bg-white/5 rounded-xl p-4 text-center">
                  ...
                  <p className="text-2xl font-bold text-white">{price ? formatPrice(price.price) : "--"}</p>
                  <div
                    className={`w-2 h-2 rounded-full mx-auto mt-2 ${price ? getStatusColor(price.status) : "bg-gray-500"}`}
                  />
                </div>
              );
            })}
...
function PriceRow({ data, showConfidence = false }: { data: PriceData; showConfidence?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-colors">
      <div className="flex items-center gap-3">
        <div
          className={`w-2 h-2 rounded-full ${data.status === "live" ? "bg-green-400" : data.status === "stale" ? "bg-yellow-400" : "bg-red-400"}`}
        />
